...
const FilterNew = (props) => {
    ...
    async function getFilterOptions() {
        ...
        if (props.genres) {
            try {
                newFilterOptions.genres =
                    (await client.service('/api/v2/genres').find()).data.map(genre => ({
                        title: genre.name,
                        value: genre._id,
                        type: 'genres'
                    }))
        ...
        for (const filter in query) {
            if (filter !== 'search' && filter !== 'page') {
                ...
                if (query[filter] && filtersOptions[filter].length > 0) {
                    ...
                    newSelectedOptions = newSelectedOptions.concat(query[filter].map(queryProp => {
                        const filterOption = filtersOptions[filter].find(option => option.value === queryProp);
                        return {
    ...
    switch (currentContent) {
        case 'search':
            currentContentElement = <SearchInput
        ...
        default:
            ...
            currentContentElement = <Options 
                options={filtersOptions[currentContent].map(option => {
                    let isSelected = false;
                    // console.log('Option')
                    // console.log(option)
                    if (selectedOptions.find(so => {return (so.type === option.type && so.value === option.value)}))
                        isSelected = true;
        ...
        if (filters[filter] && filter !== 'search') {
            filtersElements.push(
                <div
                    ...
                    className={cn(
                        styles.filter, 
                        currentContent === filter ? styles.filter_active : null)}
